<#
.SYNOPSIS
Collects OS disk partition table type (MBR/GPT) and partition details from reachable domain computers, grouped by OU.

.DESCRIPTION
This script queries Active Directory for enabled computer accounts in the current domain.
For each computer, it retrieves its OU, checks WinRM reachability using Test-WSMan.
If reachable, it remotely queries the computer to find the OS disk (marked as System disk).
It collects the disk number, partition style (MBR/GPT), number of partitions, and a comma-separated list
of partition types on that disk.
Results are outputted grouped by the computer's Organizational Unit (OU), with computers sorted alphabetically within each OU.
...
- Permissions: Read AD Computers (including DistinguishedName), Local Admin/WinRM permissions on targets.
- Active Directory PowerShell Module installed on the executing machine.
- Appropriate PowerShell Execution Policy set.

Known Limitations:
- Relies on the 'IsSystem' property of Get-Disk to identify the OS disk. This is generally reliable.
- Does not handle multi-boot scenarios explicitly; assumes the disk marked 'IsSystem' is the primary OS disk.
- Performance may vary depending on the number of computers and network latency. Consider parallel execution for very large environments.
- OU path extraction relies on standard Distinguished Name format.

.EXAMPLE
.\Get-OSDiskPartitionInfo_ByOU.ps1

Runs the script against all enabled computers in the current domain and displays the results grouped by OU.

.EXAMPLE
.\Get-OSDiskPartitionInfo_ByOU.ps1 -ExportCsvPath C:\Temp\OSDiskInfo.csv

Runs the script, displays the grouped results, and also exports the raw (ungrouped but sorted) data to a CSV file.
#>
